...
def get_report_data(filters,parent_row_group=None,indent=0,group_filter=None):
	...
	sql = sql + """ {2}
		FROM `tabSales Invoice Item` AS a
			...
			inner join `tabBin` c on c.item_code = a.item_code
		WHERE
...
def get_report_field(filters):
	if filters.parent_row_group == "Sale Invoice" or filters.row_group == "Sale Invoice" or filters.parent_row_group == "Sale Partner" or filters.row_group == "Sale Partner" :
		return [
			...
			{"label":"Profit After Commission", "short_label":"PAF.", "fieldname":"profit_after_commission","fieldtype":"Currency","indicator":"Green","precision":None, "align":"right","chart_color":"#FF3D00","sql_expression":"a.net_amount - (a.qty*c.valuation_rate*a.conversion_factor) - b.total_commission/b.total_qty"}
		]
	else:
		return [
