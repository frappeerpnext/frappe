...
def get_report_data(filters,parent_row_group=None,indent=0,group_filter=None):
	...
	for rf in report_fields:
		...
		if not hide_columns or  rf["label"] not in hide_columns:
			sql = sql + " ,{}+0.000001 AS 'total_{}' ".format(rf["sql_expression"],rf["fieldname"])
...
def get_report_summary(data,filters):
	...
	for f in fields:
		if not hide_columns or  f["label"] not in hide_columns:
			...
			if f["fieldname"] != "margin" and f["fieldname"] != "markup":
...
def get_report_field(filters):
	if filters.parent_row_group == "Sale Invoice" or filters.row_group == "Sale Invoice" or filters.parent_row_group == "Sale Partner" or filters.row_group == "Sale Partner" :
		return [
			...
			{"label":"Commission", "short_label":"Comm.", "fieldname":"total_commission","fieldtype":"Currency","indicator":"Green","precision":None, "align":"right","chart_color":"#FF3D00","sql_expression":"b.total_commission/b.total_qty"},
			{"label":"Profit After Commission", "short_label":"PAF.", "fieldname":"profit_after_commission","fieldtype":"Currency","indicator":"Green","precision":None, "align":"right","chart_color":"#FF3D00","sql_expression":"a.net_amount - (a.qty*if(coalesce(c.valuation_rate,a.incoming_rate)>a.rate,(SELECT d.valuation_rate FROM `tabItem` d WHERE d.item_code = a.item_code),coalesce(c.valuation_rate,a.incoming_rate))*a.conversion_factor) - b.total_commission/b.total_qty"}
		]
		...
		return [
			...
			{"label":"Amount", "short_label":"Amt", "fieldname":"amount","fieldtype":"Currency","indicator":"Red","precision":None, "align":"right","chart_color":"#2E7D32","sql_expression":"sum(a.net_amount)"},
			{"label":"Profit", "short_label":"Prof.", "fieldname":"profit","fieldtype":"Currency","indicator":"Green","precision":None, "align":"right","chart_color":"#FF3D00","sql_expression":"sum(a.net_amount - (a.qty*if(coalesce(c.valuation_rate,a.incoming_rate)>a.rate,(SELECT d.valuation_rate FROM `tabItem` d WHERE d.item_code = a.item_code),coalesce(c.valuation_rate,a.incoming_rate))*a.conversion_factor))"},
			{"label":"Margin", "short_label":"Marg.", "fieldname":"margin","fieldtype":"Percent","indicator":"Yellow","precision":None, "align":"right","chart_color":"#FF3D00","sql_expression":"coalesce(sum(a.net_amount - (a.qty*if(coalesce(c.valuation_rate,0)>a.rate,(SELECT d.valuation_rate FROM `tabItem` d WHERE d.item_code = a.item_code),coalesce(c.valuation_rate,a.incoming_rate))*a.conversion_factor))/sum(a.net_amount),0)*100"},
			{"label":"Markup", "short_label":"Mark.", "fieldname":"markup","fieldtype":"Percent","indicator":"Orage","precision":None, "align":"right","chart_color":"#FF3D00","sql_expression":"coalesce(sum(a.net_amount - (a.qty*if(coalesce(c.valuation_rate,0)>a.rate,(SELECT d.valuation_rate FROM `tabItem` d WHERE d.item_code = a.item_code),coalesce(c.valuation_rate,a.incoming_rate))*a.conversion_factor))/sum((a.qty*if(coalesce(c.valuation_rate,a.incoming_rate)>a.rate,(SELECT d.valuation_rate FROM `tabItem` d WHERE d.item_code = a.item_code),coalesce(c.valuation_rate,a.incoming_rate))*a.conversion_factor)),0)*100"}
		]
	else:
		return [
			{"label":"Transaction","short_label":"Tran.", "fieldname":"transaction","fieldtype":"Float", "indicator":"Grey","precision":2, "align":"center","chart_color":"#f030fd","sql_expression":"a.total_transaction"},
